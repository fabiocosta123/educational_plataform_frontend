...
export default function StudentsPage() {
  ...
  const visibleStudents = showAll ? filteredStudents : filteredStudents.slice(0, 6);
          ...
          <Card key={student.id} className="shadow-sm rounded-lg">
            ...
            <CardFooter className="flex gap-3">
              ...
              <Button
                ...
                onClick={() => {
                  const currentCourseId = student.courseEnrolled?.[0]?.courseId;

                  if (!currentCourseId) {
                    toast.error("Aluno não possui matrícula em um curso.");
                    return;
                  }

                  router.push(
                    `/dashboard-coordinator/students/${student.id}/edit?courseId=${currentCourseId}`
                  );
                }}
              >
